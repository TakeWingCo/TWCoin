...
contract ZToken is owned, SafeMath, ZSLMerkleTree {
    ...
    // Allowed for sending from first address by second address.
    ...
    // Address for Evolution Bank of Kyrgyzstan.
    address private evolutionBank;

    // Address for Central Bank of Kyrgyzstan.
    ...
    event LogMint(uint256 amount);
    event LogSterilize(uint amount);
    ...
    event LogApproval(address indexed from, address indexed spender, uint value);
    ...
    function mint(uint256 amount) onlyOwner {
        balanceOf[evolutionBank] = safeAdd(balanceOf[evolutionBank], amount);
        totalSupply = safeAdd(totalSupply, amount);
        LogMint(amount);
    }

    // Sterilize some tokens.
    function sterilize(uint amount) onlyOwner
    {
        balanceOf[centralBank] = safeSubtract(balanceOf[centralBank], amount);
	...
	function transferFrom(address _from, address _to, uint _value) public returns (bool success)
	{
		...
		if (allowance < _value || balanceOf[_from] < _value)// Check balance.
		{
			return false;
		}
